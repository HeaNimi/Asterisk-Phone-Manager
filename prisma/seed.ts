...
async function main() {
  ...
  if (!count)
    phone = await prisma.phone.create({
      data: {
        ...
        lineKeys: {
          create: [
            { position: 1, label: "Lobby", extension: "100", feature: "line" },
          ],
        },
      },
    });
  ...
  if (!contacts)
    await prisma.contact.createMany({
      data: [
        { name: "Reception", number: "100" },
        { name: "Emergency", number: "911", description: "Emergency services" },
      ],
    });
}
