...
async function create() {
  ...
  try {
    await $fetch("/api/auth/setup", {
      method: "POST",
      body: { username: setupForm.username, password: setupForm.password },
    });
    await navigateTo("/");
  } catch (e: any) {
...
onMounted(async () => {
  try {
    setupAvailable.value = (
      await $fetch<{ available: boolean }>("/api/auth/setup")
    ).available;
  } catch {
  ...
  <main class="login-layout">
    <section class="card login-card">
      ...
      <form v-else class="grid" @submit.prevent="create">
        <label
          >Admin username<input v-model="setupForm.username" required
        /></label>
        <label
          >Password<input
