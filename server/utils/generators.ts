...
const text = (value: unknown, fallback = "") =>
  xmlEscape(String(value ?? fallback));
...
const prettyXml = (xml: string) => {
  ...
  return xml
    ...
    .map((line) => {
      ...
      const formatted = `${"  ".repeat(Math.max(depth, 0))}${tag}`;
      const isOpeningTag =
        tag.startsWith("<") &&
        !tag.startsWith("</") &&
        !tag.startsWith("<?") &&
...
const featureId: Record<string, string> = {
  line: "9",
  "speed-dial": "2",
  blf: "20",
  park: "21",
...
export function ciscoConfig(
  phone: Phone,
  server: string,
  global: CiscoGlobalConfig = {},
) {
  const mac = phone.macAddress.replace(/[^A-Fa-f0-9]/g, "").toUpperCase();
  const primary =
    phone.callManagerPrimary && phone.callManagerPrimary !== "asterisk"
      ? phone.callManagerPrimary
      : server;
  const failover = phone.callManagerFailover || "";
  const managers = [primary, failover].filter(
    (value, index, list) => value && list.indexOf(value) === index,
  );
  const managerXml = managers
    .map(
      (host, index) =>
        `<member priority="${index}"><callManager><name>${text(host)}</name><ports><ethernetPhonePort>2000</ethernetPhonePort></ports></callManager></member>`,
    )
    .join("");
  const lines = (
    phone.lineKeys?.length
      ? phone.lineKeys
      : [
          {
            position: 1,
            label: phone.displayName || phone.extension,
            extension: phone.extension,
            feature: "line",
            ringSetting: phone.ringSetting || null,
          },
        ]
  )
  ...
  const directoryUrl =
    global.directoryEnabled === false
      ? ""
      : global.directoryUrl || `http://${primary}/api/directory`;
  return prettyXml(`<?xml version="1.0" encoding="UTF-8"?>
<device>
<fullConfig>${global.fullConfig === false ? "false" : "true"}</fullConfig>
...
<loadInformation>${text(global.loadInformation || phone.model)}</loadInformation>${global.inactiveLoadInformation ? `<inactiveLoadInformation>${text(global.inactiveLoadInformation)}</inactiveLoadInformation>` : ""}
<dateTimeSetting><dateTemplate>M/D/YA</dateTemplate><timeZone>${text(global.timezone || phone.timezone, "Etc/UTC")}</timeZone>${global.ntpServer || phone.ntpServer ? `<ntps><ntp><name>${text(global.ntpServer || phone.ntpServer)}</name></ntp></ntps>` : ""}</dateTimeSetting>
<callManagerGroup><members>${managerXml}</members><selectionOrder>0</selectionOrder></callManagerGroup>
<directoryURL>${text(directoryUrl)}</directoryURL><messagesURL>${text(global.messagesUrl || "")}</messagesURL><servicesURL>${text(global.servicesUrl || "")}</servicesURL>
...
export function pjsipConfig(accounts: SipAccount[]) {
  return accounts
    .filter((p) => p.username !== null || p.extension)
    .map(
      (p) =>
...
export function extensionsConfig(accounts: SipAccount[]) {
  return `[phones]\n${accounts
    ...
    .map(
      (p) =>
        `exten => ${p.extension},1,Dial(PJSIP/${p.extension},20)\n same => n,Hangup()`,
    )
    .join("\n")}\n`;
}
