...
export default defineEventHandler(async (event: any) => {
  ...
  if (event.method === 'GET') {
    ...
    const extensions = await prisma.extension.findMany({ orderBy: { extension: 'asc' }, include: { phone: { select: phoneSelect } } })
  ...
  if (event.method === 'POST') {
    ...
    if (await prisma.extension.findUnique({ where: { extension } })) {
      throw createError({ statusCode: 409, statusMessage: 'Extension number is already in use' })
    }
    const name = asString(body.name || body.displayName || `Extension ${extension}`, 120)
    const secret = asPattern(body.secret || `${extension}-change-me`, /^[^\r\n]{1,120}$/, 120)
    const phone = body.phoneId ? await prisma.phone.findUnique({ where: { id: body.phoneId }, select: phoneSelect }) : null
    if (body.phoneId && !phone) throw createError({ statusCode: 400, statusMessage: 'Phone not found' })
    const created = await prisma.$transaction(async tx => {
      if (phone) {
        await tx.extension.updateMany({ where: { phoneId: phone.id }, data: { phoneId: null } })
        await tx.phone.update({
          ...
          data: {
            ...
            displayName: body.displayName ? asString(body.displayName, 120) : phone.displayName
          }
        })
      }
      return tx.extension.create({
        data: {
          extension,
          name,
          displayName: body.displayName ? asString(body.displayName, 120) : null,
          secret,
          username: body.username ? asString(body.username, 80) : extension,
